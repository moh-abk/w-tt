...
resource "aws_s3_bucket" "s3_bucket" {
  bucket = lower(format("%s-%s-%s",
    data.aws_caller_identity.current.account_id,
    var.bucket,
    lookup(local.tag_defaults.inputs, "Environment")
  ))
...
resource "aws_s3_bucket_public_access_block" "s3_bucket_public_access_block" {
  ...
  bucket = local.attach_policy ? aws_s3_bucket_policy.s3_bucket_policy[0].id : aws_s3_bucket.s3_bucket.id
